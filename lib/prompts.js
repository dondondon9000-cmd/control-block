...
You may sometimes be given "possible gaps" between their stated plan and what's actually come up lately. These are observations, not verdicts — check in about at most one of them, naturally, only if it feels relevant right now, never as an interrogation and never in every message. Whether or not one was raised, only fill in a plan update if the user explicitly tells you, in their own words, that their goals or direction have genuinely changed and they want it updated — never because they seem busy, discouraged, avoidant, or had one hard day. That's a high bar on purpose: you steer people back toward their stated goals by default, and only change the goal itself when they've actually said so.
  ...
  input_schema: {
    ...
    properties: {
      ...
      raisedPlanGap: {
        type: 'boolean',
        description:
          'True only if this reply actually raised one of the "possible gaps" given to you below your instructions. False otherwise, including when none were given or none felt right to bring up now.',
      },
      planUpdate: {
        type: 'object',
        description:
          'Only set changed:true if, in this exact message, the user explicitly confirmed their goals/direction/plan have genuinely changed and they want it updated. Being busy, discouraged, avoidant, or not mentioning the plan does NOT count — this is a high bar. Leave every field empty/false otherwise.',
        properties: {
          changed: { type: 'boolean' },
          vision: {
            type: 'string',
            description: 'Revised future vision in their own words/spirit. Empty string if changed is false.',
          },
          obstacles: {
            type: 'string',
            description: 'Revised obstacles to match. Empty string if changed is false.',
          },
          steps: {
            type: 'array',
            items: { type: 'string' },
            description: '3-5 revised concrete growth-plan steps. Empty array if changed is false.',
          },
        },
        required: ['changed', 'vision', 'obstacles', 'steps'],
      },
    },
    required: [
      'reply',
  ...
  input_schema: {
    ...
    properties: {
      ...
      planGapNotes: {
        ...
        items: { type: 'string' },
        description:
          "Only for monthly summaries: 0-3 short, neutral observations about a specific gap between the user's currently stated future vision/obstacles/growth plan (given to you below) and what actually came up this month — e.g. a plan step never once mentioned, or a new direction hinted at repeatedly. You are NOT deciding whether the plan is wrong or should change — that's the user's call, made in conversation, not yours to declare here. Phrase each as something worth checking in on, not a verdict. Empty array if the plan and this month's entries are well aligned, if no vision has been set yet, or for weekly summaries.",
      },
    },
    required: [
      'mainEmotions',
...
export function periodSummarySystemPrompt(kind) {
  ...
  if (kind === 'monthly') {
    prompt += ` You'll also be given the user's currently stated future vision, obstacles, and growth plan from when they set this up. Compare it against what actually came up this month, but you are not deciding whether to change it — that decision belongs to the user, made in conversation, not to you here. Just note, in planGapNotes, any specific and concrete gap worth checking in on (a plan step never once mentioned, a new direction hinted at repeatedly) — phrased neutrally, not as a verdict. Leave it empty if things are aligned, or if there's nothing concrete enough to point to.`;
  }
